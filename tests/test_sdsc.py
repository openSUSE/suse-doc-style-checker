...
def test_xml(xmltestcase):
    ...
    for id in inputids:
        ...
        # "a-an" ]
        findExpect = re.search('.*sdsc\.expect\.([^.]+)\.([^.]+)(?:\..*|$)', id)
        if not findExpect:
            continue
        found = False
        if findExpect.group(2) in complaints:
            found = sum(complaint["id"] == id and complaint["type"] == findExpect.group(
                        1) for complaint in complaints[findExpect.group(2)])

        if not found:
            print("Expected {0} for ID {1!r} generated by module {2!r}.".format(
                findExpect.group(1), id, findExpect.group(2)), file=sys.stderr)
            nr_errors += 1
