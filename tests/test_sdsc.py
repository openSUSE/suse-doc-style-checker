...
def test_xml(xmltestcase):
    ...
    for checkmodule, complaintList in complaints.items():
        for complaint in complaintList:
            ...
            if not (complaint["id"].startswith("sdsc.expect.{0}.{1}".format(complaint["type"], checkmodule)) or
                    complaint["id"].startswith("override.sdsc.expect.{0}.{1}".format(complaint["type"], checkmodule)) or
                    complaint["id"].startswith("sec.override.sdsc.expect.{0}.{1}".format(complaint["type"], checkmodule))):
